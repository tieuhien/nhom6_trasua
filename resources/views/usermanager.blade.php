...
@else
    <div class="container1">
       <div class="login">Quản lý thành viên</div>




<div style="position: absolute;right: 50px;">
   <a href="register" class="submit"  title="thêm thành viên" style="padding:7px;background-color: #4dbd4dad !important;">Thêm thành viên</a>  
   ...
   <a href="{{ url()->previous() }}" class="submit"  title="trở về trang trước đó" style="padding:7px;background-color: #8a5f2db0 !important;">Quay lại</a>  
        ...
        <table border="1">
            <tr>
                ...
                <th>Địa chỉ:</th>
                <th>Ngày tạo:</th>
                <th title=" 0 là người dùng bình thường 1 là admin">Chức vụ</th>
            ...
            <tr>  
                ...
                <td>{{$sp->name}}</td>
                <td><a href="mailto:{{$sp->email}}">{{$sp->email}}</a></td>
                <td><a href="tel:{{$sp->phone}}" >{{$sp->phone}}</a></td>
                <td>{{$sp->address}}</td>
                <td>{{$sp->created_at}}</td>
                <td>{{$sp->position}}</td>
                <td class="center">
                    <form method="get" action="UsersController/{{$sp->id}}">
                    <button class="submit" type="submit" name="show" title="Có gì đó sai sai >> chỉnh sửa luôn." >Sửa</button> 
                   </form>
                   
                </td>
                <td class="center">
                    <form action="UsersController/{{$sp->id}}" method="post">
        ...
        <div id="show">Hiển thị {{$users->firstItem()}}-{{$users->perPage()}} của 
       {{$users->total()}} người dùng. </div>



  <ul class="pagi">
        <li class="pagi-item pagi-action pagi-prev is-disabled">
          <a href="{{$users->previousPageUrl()}}"><i class="fal fa-angle-left"></i></a>
        </li>
        <?php for($i=1;$i<=$users->lastPage();$i++){ ?>
        <a href="{{$users->url($i)}}"><li class="pagi-item"><?php echo $i;?> </li></a>
       <?php }; ?>

       <li class="pagi-item pagi-action pagi-next">
          <a href="{{$users->nextPageUrl()}}"> <i class="fal fa-angle-right"></i></a>
        </li>
      </ul>
